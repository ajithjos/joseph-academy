...
2. Build the static artifacts on the host:
   ...
   - `cd docs_site && npm install && npm run build`
   - `cd fe/flutter/apps/joseph_academy && flutter pub get && flutter build web --release --dart-define=JOSEPH_ACADEMY_API_BASE_URL=http://YOUR_VM_IP:8788`
...
- static Docusaurus docs build served by nginx
