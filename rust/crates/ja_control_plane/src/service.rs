...
use ja_catalog::{CatalogBundle, CatalogValidationReport, PlanTemplate, load_bootstrap, load_catalog_bundle};
use serde_json::json;
...
pub async fn apply_bootstrap(state: &Arc<AppState>) -> anyhow::Result<BootstrapApplyResponse> {
    ...
    for user in &bootstrap.users {
        query(
            "insert into user_account (user_id, username, display_name) values ($1, $2, $3)
    ...
    for learner in &bootstrap.learners {
        query(
            "insert into learner (learner_id, team_id, user_id, display_name, date_of_birth, sex, current_level, notes)
        ...
        .bind(&learner.team_id)
        .bind(&learner.user_id)
        .bind(&learner.display_name)
        .bind(learner.date_of_birth)
        .bind(&learner.sex)
        .bind(&learner.current_level)
        .bind(&learner.notes)
        .execute(&state.pool)
        .await?;
    }
    ...
    Ok(BootstrapApplyResponse {
        ...
        learner_count: bootstrap.learners.len(),
        seeded_plan_count,
    })
}
...
pub async fn rebuild_review_queue(
    ...
    let learner_ids = if let Some(learner_id) = learner_id {
        ...
        query_scalar::<_, String>("select learner_id from learner order by learner_id")
            .fetch_all(&state.pool)
            .await?
    };
...
fn choose_content_for_capability<'a>(
    ...
    session: &'a ja_catalog::PlanTemplateSession,
    capability_id: &str,
) -> Option<&'a str> {
